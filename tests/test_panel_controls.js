...
assert.match(firmware, /constexpr float LINK_1_MM = 260\.0f;/);
assert.match(firmware, /constexpr float LINK_2_MM = 255\.0f;/);
...
assert.match(
  firmware,
  /on\[i\] \? \(i == 3 \? LedColor::WHITE : LedColor::GREEN\)\s+: LedColor::OFF;/,
);
assert.match(firmware, /LedColor::WHITE \? CRGB::White : CRGB::Black/);
