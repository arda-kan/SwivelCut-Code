...
assert.ok(Math.abs(autoNegative.j1Deg + 27.122703086070917) < 1e-9);
assert.ok(Math.abs(autoNegative.j2Deg + 148.14201621003818) < 1e-9);
let autoPositive = inverseKinematicsAuto(100, 100);
assert.ok(autoPositive);
assert.strictEqual(autoPositive.elbowDown, false);
assert.ok(Math.abs(autoPositive.j1Deg - 27.122703086070917) < 1e-9);
assert.ok(Math.abs(autoPositive.j2Deg - 148.14201621003818) < 1e-9);
assert.strictEqual(inverseKinematics(516, 0, false), null);
