...
assert.ok(firmware.includes('command == "BLADE STATUS"'));
...
assert.ok(firmware.includes("ledcAttachChannel("));
assert.ok(firmware.includes("ledcWriteChannel(BLADE_PWM_CHANNEL, duty);"));
assert.ok(firmware.includes("ERROR_BLADE_RELAY_OFF"));
assert.ok(firmware.includes("ERROR_BLADE_PWM_NOT_READY"));
assert.ok(firmware.includes("BLADE_DRIVE PWM_GPIO="));
